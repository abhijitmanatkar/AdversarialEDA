...
class Repository:
    ...
    def add_back_actions_to_session(self, session_df):
        ...
        for idx, back_rows_lst in back_rows.items():
            dfs_to_concat.append(session_df.loc[start_idx:idx - 1])
            for back_row in back_rows_lst:
                dfs_to_concat.append(back_row)
            start_idx = idx
        dfs_to_concat.append(session_df.loc[start_idx:])
    ...
    def convert_actions_to_atena(self):
        ...
        flawed_session_ids = set(
            [20, 56, 85, 94, 106, 129, 133, 177, 277, 303, 331, 370, 388, 407, 468, 489, 516, 522, 582, 603, 635,
             723] + list(range(672, 720)) + [345, 405, 408, 498] + [671])
        
        # sessions [345, 405, 408, 498] are flawed because they have display id's which
        # don't match with any valid entry in the raw_datasets

        # session number 671 is flawed because it contains groupings on fields like Country, etc. 
        # which aren't present in the original schema
