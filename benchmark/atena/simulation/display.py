...
from benchmark.atena.simulation.actions import FilterOperator, AggregationFunction
from benchmark.atena.simulation.dataset import Dataset
from benchmark.atena.simulation.state import FilteringTuple, AggregationTuple, DisplayTuple
from benchmark.atena.utils.data_structures import CounterWithoutNanKeys
...
class DisplayCalculator(object):
    ...
    def get_filtered_df(self, filter_tpl: FilteringTuple):
        ...
        for filt in filter_tpl:
            column = field = filt.column
            filter_operator = opr = filt.operator
            filter_term = value = filt.term
            ...
            # if filter_operator in {FilterOperator.EQUAL, FilterOperator.NOTEQUAL, FilterOperator.GT, FilterOperator.GE}:
            #     try:
